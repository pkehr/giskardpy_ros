...
class WorldWithHSRAndTurtleConfig(WorldConfig):
    map_name: str
    ...
    def setup(self):
        ...
        self.world.links[turtle_odom_link_name].collisions.append(
            CylinderGeometry(height=0.62, radius=0.25, color=ColorRGBA(1, 1, 1, 1)))

        self.add_6dof_joint(parent_link=self.map_name, child_link=turtle_odom_link_name,
                            joint_name=PrefixName('turtle_odom_joint', 'turtle'))
...
class SuturoArenaWithHSRAndTurtleConfig(WorldWithHSRAndTurtleConfig):
    ...
    def setup(self):
        ...
        self.add_fixed_joint(parent_link=self.map_name, child_link=root_link_name,
                             homogenous_transform=msg_converter.ros_msg_to_giskard_obj(kitchen_pose.pose,
                                                                                       god_map.world))


class SuturoArenaWithHSRWithTurtleBotConfig(WorldWithHSRAndTurtleConfig):
    ...
    def setup(self):
        ...
        self.world.links[link_name].collisions.append(
            CylinderGeometry(height=0.01, radius=0.25, color=ColorRGBA(1, 1, 1, 1)))
        self.add_6dof_joint(parent_link=self.map_name, child_link=link_name,
                            joint_name=PrefixName('turtle_joint', 'turtle'))

        self.world.register_group('turtle', root_link_name=link_name)
...
class HSRVelocityInterfaceSuturo(HSRVelocityInterface):
    ...
    def __init__(self, map_name: str = 'map', localization_joint_name: str = 'localization',
                 ...
                 environment_name: str = 'iai_kitchen', sync_turtle=False):
    ...
    def setup(self):
        ...
        if self.sync_turtle:
            self.sync_6dof_joint_with_tf_frame(joint_name=PrefixName('turtle_joint', 'turtle'), tf_parent_frame='map',
                                               tf_child_frame='turtle/base_footprint')
